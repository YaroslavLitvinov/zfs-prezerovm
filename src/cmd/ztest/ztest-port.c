...
vdev_channel_size_is_valid()
{
    ...
    struct stat st;
    int error = stat64(dev_name, &st);
    return st.st_size < SPA_MINDEVSIZE? B_FALSE: B_TRUE;
}
...
#ifdef __native_client__
    /*channel was closed in kernel_fini*/
...
ztest_init(char *pool)
{
    ...
    ztest_walk_pool_directory("anytext");
